...
(Félix Cloutier, 2019), [https://www.felixcloutier.com/x86/cmp](https://www.felixcloutier.com/x86/cmp) [↩︎](https://portal.offsec.com/courses/pen-300-9502/learning/advanced-antivirus-evasion-14696/advanced-antivirus-evasion-15017#fnref-local_id_308-7)
...
At the time of this writing, only 11 antivirus vendors currently support AMSI,[1](https://portal.offsec.com/courses/pen-300-9502/learning/advanced-antivirus-evasion-14696/advanced-antivirus-evasion-15017#fn-local_id_310-1) which means that the content passed by AMSI is only analyzed if one of those antivirus products is installed. More antivirus vendors will provide support over time, but it should be noted that AMSI was first introduced in the release of Windows 10 in 2015 so the third-party adoption rate has not been impressive.
...
(Wikipedia, 2020), [https://en.wikipedia.org/wiki/Zero\_flag](https://en.wikipedia.org/wiki/Zero_flag) [↩︎](https://portal.offsec.com/courses/pen-300-9502/learning/advanced-antivirus-evasion-14696/advanced-antivirus-evasion-15017#fnref-local_id_317-1)
...
(winscripting.blog, 2017), [https://winscripting.blog/2017/05/12/first-entry-welcome-and-uac-bypass/](https://winscripting.blog/2017/05/12/first-entry-welcome-and-uac-bypass/) [↩︎](https://portal.offsec.com/courses/pen-300-9502/learning/advanced-antivirus-evasion-14696/advanced-antivirus-evasion-15017#fnref-local_id_320-1)

2

(Microsoft, 2018), [https://docs.microsoft.com/en-us/powershell/module/microsoft.powershell.management/new-item](https://docs.microsoft.com/en-us/powershell/module/microsoft.powershell.management/new-item) [↩︎](https://portal.offsec.com/courses/pen-300-9502/learning/advanced-antivirus-evasion-14696/advanced-antivirus-evasion-15017#fnref-local_id_320-2)

3

(Microsoft, 2020), [https://docs.microsoft.com/en-us/powershell/module/microsoft.powershell.management/new-itemproperty?view=powershell-6](https://docs.microsoft.com/en-us/powershell/module/microsoft.powershell.management/new-itemproperty?view=powershell-6) [↩︎](https://portal.offsec.com/courses/pen-300-9502/learning/advanced-antivirus-evasion-14696/advanced-antivirus-evasion-15017#fnref-local_id_320-3)
...
<figure><img src="https://static.offsec.com/offsec-courses/PEN-300/imgs/aav/796ff4767bcfa6b04832750178a33ef4-aav_uac_meter.png" alt="Figure 9: Antivirus alert from Windows Defender due to Meterpreter payload"><figcaption><p>Figure 9: Antivirus alert from Windows Defender due to Meterpreter payload</p></figcaption></figure>
...
We could avoid this by enabling the advanced _EnableStageEncoding_ option along with _StageEncoder_ in Metasploit. We'll set _EnableStageEncoding_ to "true" and _StageEncoder_ to a compatible encoder, in this case _x64/zutto\_dekiru_. If this fails we suggesting using _x64/xor\_dynamic_.

Now let's try to exploit this using x64/zutto\_dekiru:
...
(Microsoft, 2018), [https://docs.microsoft.com/en-us/windows/win32/sysinfo/registry-element-size-limits](https://docs.microsoft.com/en-us/windows/win32/sysinfo/registry-element-size-limits) [↩︎](https://portal.offsec.com/courses/pen-300-9502/learning/advanced-antivirus-evasion-14696/advanced-antivirus-evasion-15017#fnref-local_id_321-1)
...
First, we paused execution for 20 seconds with the _Sleep_[1](https://portal.offsec.com/courses/pen-300-9502/learning/advanced-antivirus-evasion-14696/advanced-antivirus-evasion-15017#fn-local_id_323-1) method. This delay helps us identify the process ID of the wscript.exe process with Process Explorer, start the frida-trace command, and allow it to hook the APIs.
...
(SS64, 2020), [https://ss64.com/vb/sleep.html](https://ss64.com/vb/sleep.html) [↩︎](https://portal.offsec.com/courses/pen-300-9502/learning/advanced-antivirus-evasion-14696/advanced-antivirus-evasion-15017#fnref-local_id_323-1)
...
Security researcher @Tal\_Liberman discovered that Jscript tries to query the "AmsiEnable" registry key from the HKCU hive before initializing AMSI.[1](https://portal.offsec.com/courses/pen-300-9502/learning/advanced-antivirus-evasion-14696/advanced-antivirus-evasion-15017#fn-local_id_324-1) If this key is set to "0", AMSI is not enabled for the Jscript process.
...
<figure><img src="https://static.offsec.com/offsec-courses/PEN-300/imgs/aav/302c6768115933ce11841770eb68d45e-aav_jscript_start.png" alt="Figure 10: Starting wscript.exe from WinDbg"><figcaption><p>Figure 10: Starting wscript.exe from WinDbg</p></figcaption></figure>
...
A subsequent call to _RegQueryValueExW_[3](https://portal.offsec.com/courses/pen-300-9502/learning/advanced-antivirus-evasion-14696/advanced-antivirus-evasion-15017#fn-local_id_324-3) highlighted in Listing 64 is used to query the registry value. The name of the registry key is also supplied as the second argument (RDX) to this API so we can dump it in WinDbg:
...
We now have the full path to the registry key. In order to bypass AMSI, we'll create the key and set its value to "0" with the _RegWrite_[4](https://portal.offsec.com/courses/pen-300-9502/learning/advanced-antivirus-evasion-14696/advanced-antivirus-evasion-15017#fn-local_id_324-4) method from the _WScript.Shell_ object. This method accepts the full registry key, the value content, and the value data type as shown in the Jscript code below:
...
The full code, excluding the shellcode runner itself, is shown in Listing 69.[5](https://portal.offsec.com/courses/pen-300-9502/learning/advanced-antivirus-evasion-14696/advanced-antivirus-evasion-15017#fn-local_id_324-5)
...
Let's unpack a few elements of this code. First, the code is wrapped in _try_ and _catch_ exception handling statements.[6](https://portal.offsec.com/courses/pen-300-9502/learning/advanced-antivirus-evasion-14696/advanced-antivirus-evasion-15017#fn-local_id_324-6)
...
Inside the _try_ statement, we call _RegRead_[7](https://portal.offsec.com/courses/pen-300-9502/learning/advanced-antivirus-evasion-14696/advanced-antivirus-evasion-15017#fn-local_id_324-7) to determine if the AmsiEnable key is already set. If it isn't, the _throw_[8](https://portal.offsec.com/courses/pen-300-9502/learning/advanced-antivirus-evasion-14696/advanced-antivirus-evasion-15017#fn-local_id_324-8) statement along with the _new Error_[9](https://portal.offsec.com/courses/pen-300-9502/learning/advanced-antivirus-evasion-14696/advanced-antivirus-evasion-15017#fn-local_id_324-9) constructor throws a new exception. If this happens, the code inside the _catch_ statement is executed, setting the AmsiEnable value and invoking the _Run_[10](https://portal.offsec.com/courses/pen-300-9502/learning/advanced-antivirus-evasion-14696/advanced-antivirus-evasion-15017#fn-local_id_324-10) method.

The arguments for the call to the _Run_ method are important. First, we specify the _cscript.exe_[11](https://portal.offsec.com/courses/pen-300-9502/learning/advanced-antivirus-evasion-14696/advanced-antivirus-evasion-15017#fn-local_id_324-11) executable, which is the command-line equivalent of wscript.exe.

Next, we use -e to specify which scripting engine will execute the script. The highlighted value in Listing 69 is a _globally unique identifier_ (GUID),[12](https://portal.offsec.com/courses/pen-300-9502/learning/advanced-antivirus-evasion-14696/advanced-antivirus-evasion-15017#fn-local_id_324-12) which when used in this manner may be understood as a registry entry under HKLM\SOFTWARE\Classes\CLSID.
...
<figure><img src="https://static.offsec.com/offsec-courses/PEN-300/imgs/aav/0c506f9248f599c7b1dea498e3075451-aav_jscript_guid.png" alt="Figure 11: GUID registry entry for jscript.dll"><figcaption><p>Figure 11: GUID registry entry for jscript.dll</p></figcaption></figure>

In essence, the -e option indicates that the specified script file will be processed by jscript.dll.

The script file must be the original Jscript and we provide this through the _ScriptFullName_[13](https://portal.offsec.com/courses/pen-300-9502/learning/advanced-antivirus-evasion-14696/advanced-antivirus-evasion-15017#fn-local_id_324-13) property as shown in Listing 70, where we repeat the _Run_ method.
...
(Dominic Shell, 2019), [https://hackinparis.com/data/slides/2019/talks/HIP2019-Dominic\_Chell-Cracking\_The\_Perimeter\_With\_Sharpshooter.pdf](https://hackinparis.com/data/slides/2019/talks/HIP2019-Dominic_Chell-Cracking_The_Perimeter_With_Sharpshooter.pdf) [↩︎](https://portal.offsec.com/courses/pen-300-9502/learning/advanced-antivirus-evasion-14696/advanced-antivirus-evasion-15017#fnref-local_id_324-1)

2

(Microsoft, 2018), [https://docs.microsoft.com/en-us/windows/win32/api/winreg/nf-winreg-regopenkeyexw](https://docs.microsoft.com/en-us/windows/win32/api/winreg/nf-winreg-regopenkeyexw) [↩︎](https://portal.offsec.com/courses/pen-300-9502/learning/advanced-antivirus-evasion-14696/advanced-antivirus-evasion-15017#fnref-local_id_324-2)

3

(Microsoft, 2018), [https://docs.microsoft.com/en-us/windows/win32/api/winreg/nf-winreg-regqueryvalueexw](https://docs.microsoft.com/en-us/windows/win32/api/winreg/nf-winreg-regqueryvalueexw) [↩︎](https://portal.offsec.com/courses/pen-300-9502/learning/advanced-antivirus-evasion-14696/advanced-antivirus-evasion-15017#fnref-local_id_324-3)

4

(SS64, 2020), [https://ss64.com/vb/regwrite.html](https://ss64.com/vb/regwrite.html) [↩︎](https://portal.offsec.com/courses/pen-300-9502/learning/advanced-antivirus-evasion-14696/advanced-antivirus-evasion-15017#fnref-local_id_324-4)

5

(MDSec, 2019), [https://github.com/mdsecactivebreach/SharpShooter/blob/master/modules/amsikiller.py](https://github.com/mdsecactivebreach/SharpShooter/blob/master/modules/amsikiller.py) [↩︎](https://portal.offsec.com/courses/pen-300-9502/learning/advanced-antivirus-evasion-14696/advanced-antivirus-evasion-15017#fnref-local_id_324-5)

6

(W3Schools, 2020), [https://www.w3schools.com/js/js\_errors.asp](https://www.w3schools.com/js/js_errors.asp) [↩︎](https://portal.offsec.com/courses/pen-300-9502/learning/advanced-antivirus-evasion-14696/advanced-antivirus-evasion-15017#fnref-local_id_324-6)

7

(SS64, 2020), [https://ss64.com/vb/regread.html](https://ss64.com/vb/regread.html) [↩︎](https://portal.offsec.com/courses/pen-300-9502/learning/advanced-antivirus-evasion-14696/advanced-antivirus-evasion-15017#fnref-local_id_324-7)

8

(W3Schools, 2020), [https://www.w3schools.com/js/js\_errors.asp](https://www.w3schools.com/js/js_errors.asp) [↩︎](https://portal.offsec.com/courses/pen-300-9502/learning/advanced-antivirus-evasion-14696/advanced-antivirus-evasion-15017#fnref-local_id_324-8)

9

(Mozilla, 202), [https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global\_Objects/Error](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Error) [↩︎](https://portal.offsec.com/courses/pen-300-9502/learning/advanced-antivirus-evasion-14696/advanced-antivirus-evasion-15017#fnref-local_id_324-9)

10

(SS64, 2020), [https://ss64.com/vb/run.html](https://ss64.com/vb/run.html) [↩︎](https://portal.offsec.com/courses/pen-300-9502/learning/advanced-antivirus-evasion-14696/advanced-antivirus-evasion-15017#fnref-local_id_324-10)

11

(Microsoft, 2017), [https://docs.microsoft.com/en-us/windows-server/administration/windows-commands/cscript](https://docs.microsoft.com/en-us/windows-server/administration/windows-commands/cscript) [↩︎](https://portal.offsec.com/courses/pen-300-9502/learning/advanced-antivirus-evasion-14696/advanced-antivirus-evasion-15017#fnref-local_id_324-11)

12

(Wikipedia, 2020), [https://en.wikipedia.org/wiki/Universally\_unique\_identifier](https://en.wikipedia.org/wiki/Universally_unique_identifier) [↩︎](https://portal.offsec.com/courses/pen-300-9502/learning/advanced-antivirus-evasion-14696/advanced-antivirus-evasion-15017#fnref-local_id_324-12)

13

(SS64, 2020), [https://ss64.com/vb/syntax-wscript.html](https://ss64.com/vb/syntax-wscript.html) [↩︎](https://portal.offsec.com/courses/pen-300-9502/learning/advanced-antivirus-evasion-14696/advanced-antivirus-evasion-15017#fnref-local_id_324-13)
...
While it seems logical to attempt to simply overwrite AMSI.DLL, we must have administrative permissions to overwrite anything in C:\Windows\System32. We could, however, perform a DLL hijacking attack[1](https://portal.offsec.com/courses/pen-300-9502/learning/advanced-antivirus-evasion-14696/advanced-antivirus-evasion-15017#fn-local_id_325-1) by exploiting the DLL search order.
...
Once WinDbg has launched wscript.exe and a bare minimum of modules, it breaks the execution flow. Listing the loaded modules (lm)[2](https://portal.offsec.com/courses/pen-300-9502/learning/advanced-antivirus-evasion-14696/advanced-antivirus-evasion-15017#fn-local_id_325-2) and searching for a module named amsi (m amsi) reveals that AMSI.DLL has not yet loaded.
...
One way to accomplish this is to instruct the debugger to catch the load of the DLL in WinDbg. We can do this with the sxe[3](https://portal.offsec.com/courses/pen-300-9502/learning/advanced-antivirus-evasion-14696/advanced-antivirus-evasion-15017#fn-local_id_325-3) command along with the ld[4](https://portal.offsec.com/courses/pen-300-9502/learning/advanced-antivirus-evasion-14696/advanced-antivirus-evasion-15017#fn-local_id_325-4) subcommand to detect when a module is loaded by supplying the name as an argument.
...
Next, we need to locate the code responsible for loading AMSI.DLL. A DLL is typically loaded through the Win32 _LoadLibrary_[5](https://portal.offsec.com/courses/pen-300-9502/learning/advanced-antivirus-evasion-14696/advanced-antivirus-evasion-15017#fn-local_id_325-5) or _LoadLibraryEx_[6](https://portal.offsec.com/courses/pen-300-9502/learning/advanced-antivirus-evasion-14696/advanced-antivirus-evasion-15017#fn-local_id_325-6) APIs so we must look for that function and see what function invoked it.
...
According to the _LoadLibraryExW_[6:1](https://portal.offsec.com/courses/pen-300-9502/learning/advanced-antivirus-evasion-14696/advanced-antivirus-evasion-15017#fn-local_id_325-6) function prototype, the first argument is the name of the DLL to load. The last lines of Listing 74 reveals that the name of the DLL is "amsi.dll", listed without a full path.

This is significant considering the DLL search order.[7](https://portal.offsec.com/courses/pen-300-9502/learning/advanced-antivirus-evasion-14696/advanced-antivirus-evasion-15017#fn-local_id_325-7) When a full path is not provided, the folder of the launched application is searched first. If we copy wscript.exe to a writable location and place a custom version of AMSI.DLL in the same folder, this could open up an attack vector.

However, _LoadLibraryExW_ can accept additional arguments and the third argument modifies the function's default behavior. In this case, R8 (the third argument) is set to 0x800 (as highlighted in Listing 74). This is equivalent to the enum _LOAD\_LIBRARY\_SEARCH\_SYSTEM32_, which forces the function to search in the C:\Windows\System32 directory first.

This prevents a DLL hijacking attack. Security researcher James Forshaw discovered an interesting way around this.[8](https://portal.offsec.com/courses/pen-300-9502/learning/advanced-antivirus-evasion-14696/advanced-antivirus-evasion-15017#fn-local_id_325-8) Instead of trying to hijack the DLL loading, James suggests renaming wscript.exe to amsi.dll and executing it.
...
The second important thing to note is that double-clicking or running a file with a .dll extension will fail since DLLs are normally loaded, not executed. This behavior is actually caused by the Win32 _ShellExecute_[9](https://portal.offsec.com/courses/pen-300-9502/learning/advanced-antivirus-evasion-14696/advanced-antivirus-evasion-15017#fn-local_id_325-9) API, which is used by cmd.exe.

However, if we instead use the _CreateProcess_[10](https://portal.offsec.com/courses/pen-300-9502/learning/advanced-antivirus-evasion-14696/advanced-antivirus-evasion-15017#fn-local_id_325-10) Win32 API, the file extension is ignored and the file header would be parsed to determine if it is a valid executable. We cannot directly call this API, but we can use the _Exec_[11](https://portal.offsec.com/courses/pen-300-9502/learning/advanced-antivirus-evasion-14696/advanced-antivirus-evasion-15017#fn-local_id_325-11) method of the _WScript.Shell_ object since it's just a wrapper for it.
...
In the _try_ statement, we first detect if the copied executable already exists through the _FileExists_ method[12](https://portal.offsec.com/courses/pen-300-9502/learning/advanced-antivirus-evasion-14696/advanced-antivirus-evasion-15017#fn-local_id_325-12) of the _FileSystemObject_ object. If it does, we execute the DotNetToJscript-generated shellcode runner.

If it does not yet exist, we trigger an exception and the code in the _catch_ section is executed. Here, we use the _CopyFile_[13](https://portal.offsec.com/courses/pen-300-9502/learning/advanced-antivirus-evasion-14696/advanced-antivirus-evasion-15017#fn-local_id_325-13) method to copy wscript.exe into the C:\Windows\Tasks folder and name it "AMSI.DLL".
...
<figure><img src="https://static.offsec.com/offsec-courses/PEN-300/imgs/aav/db6aaf44e78e316a87d624e0e6fdf757-aav_jscript_own.png" alt="Figure 12: Antivirus alert due to AMSI bypass"><figcaption><p>Figure 12: Antivirus alert due to AMSI bypass</p></figcaption></figure>
...
(Mitre, 2020), [https://attack.mitre.org/techniques/T1038/](https://attack.mitre.org/techniques/T1038/) [↩︎](https://portal.offsec.com/courses/pen-300-9502/learning/advanced-antivirus-evasion-14696/advanced-antivirus-evasion-15017#fnref-local_id_325-1)

2

(Microsoft, 2017), [https://docs.microsoft.com/en-us/windows-hardware/drivers/debugger/lm--list-loaded-modules-](https://docs.microsoft.com/en-us/windows-hardware/drivers/debugger/lm--list-loaded-modules-) [↩︎](https://portal.offsec.com/courses/pen-300-9502/learning/advanced-antivirus-evasion-14696/advanced-antivirus-evasion-15017#fnref-local_id_325-2)

3

(Microsoft, 2017), [https://docs.microsoft.com/en-us/windows-hardware/drivers/debugger/sx--sxd--sxe--sxi--sxn--sxr--sx---set-exceptions-](https://docs.microsoft.com/en-us/windows-hardware/drivers/debugger/sx--sxd--sxe--sxi--sxn--sxr--sx---set-exceptions-) [↩︎](https://portal.offsec.com/courses/pen-300-9502/learning/advanced-antivirus-evasion-14696/advanced-antivirus-evasion-15017#fnref-local_id_325-3)

4

(Microsoft, 2018), [https://docs.microsoft.com/en-us/windows-hardware/drivers/debugger/ld--load-symbols-](https://docs.microsoft.com/en-us/windows-hardware/drivers/debugger/ld--load-symbols-) [↩︎](https://portal.offsec.com/courses/pen-300-9502/learning/advanced-antivirus-evasion-14696/advanced-antivirus-evasion-15017#fnref-local_id_325-4)

5

(Microsoft, 2018), [https://docs.microsoft.com/en-us/windows/win32/api/libloaderapi/nf-libloaderapi-loadlibrarya](https://docs.microsoft.com/en-us/windows/win32/api/libloaderapi/nf-libloaderapi-loadlibrarya) [↩︎](https://portal.offsec.com/courses/pen-300-9502/learning/advanced-antivirus-evasion-14696/advanced-antivirus-evasion-15017#fnref-local_id_325-5)

6

(Microsoft, 2018), [https://docs.microsoft.com/en-us/windows/win32/api/libloaderapi/nf-libloaderapi-loadlibraryexw](https://docs.microsoft.com/en-us/windows/win32/api/libloaderapi/nf-libloaderapi-loadlibraryexw) [↩︎](https://portal.offsec.com/courses/pen-300-9502/learning/advanced-antivirus-evasion-14696/advanced-antivirus-evasion-15017#fnref-local_id_325-6) [↩︎](https://portal.offsec.com/courses/pen-300-9502/learning/advanced-antivirus-evasion-14696/advanced-antivirus-evasion-15017#fnref-local_id_325-6:1)

7

(Microsoft, 2018), [https://docs.microsoft.com/en-us/windows/win32/dlls/dynamic-link-library-search-order](https://docs.microsoft.com/en-us/windows/win32/dlls/dynamic-link-library-search-order) [↩︎](https://portal.offsec.com/courses/pen-300-9502/learning/advanced-antivirus-evasion-14696/advanced-antivirus-evasion-15017#fnref-local_id_325-7)

8

(James Forshaw, 2018), [https://tyranidslair.blogspot.com/2018/06/disabling-amsi-in-jscript-with-one.html](https://tyranidslair.blogspot.com/2018/06/disabling-amsi-in-jscript-with-one.html) [↩︎](https://portal.offsec.com/courses/pen-300-9502/learning/advanced-antivirus-evasion-14696/advanced-antivirus-evasion-15017#fnref-local_id_325-8)

9

(Microsoft, 2018), [https://docs.microsoft.com/en-us/windows/win32/api/shellapi/nf-shellapi-shellexecutea](https://docs.microsoft.com/en-us/windows/win32/api/shellapi/nf-shellapi-shellexecutea) [↩︎](https://portal.offsec.com/courses/pen-300-9502/learning/advanced-antivirus-evasion-14696/advanced-antivirus-evasion-15017#fnref-local_id_325-9)

10

(Microsoft, 2018), [https://docs.microsoft.com/en-us/windows/win32/api/processthreadsapi/nf-processthreadsapi-createprocessa](https://docs.microsoft.com/en-us/windows/win32/api/processthreadsapi/nf-processthreadsapi-createprocessa) [↩︎](https://portal.offsec.com/courses/pen-300-9502/learning/advanced-antivirus-evasion-14696/advanced-antivirus-evasion-15017#fnref-local_id_325-10)

11

(SS64, 2020), [https://ss64.com/vb/exec.html](https://ss64.com/vb/exec.html) [↩︎](https://portal.offsec.com/courses/pen-300-9502/learning/advanced-antivirus-evasion-14696/advanced-antivirus-evasion-15017#fnref-local_id_325-11)

12

(SS64, 2020), [https://ss64.com/vb/filesystemobject.html](https://ss64.com/vb/filesystemobject.html) [↩︎](https://portal.offsec.com/courses/pen-300-9502/learning/advanced-antivirus-evasion-14696/advanced-antivirus-evasion-15017#fnref-local_id_325-12)

13

(SS64, 2020), [https://ss64.com/vb/filesystemobject.html](https://ss64.com/vb/filesystemobject.html) [↩︎](https://portal.offsec.com/courses/pen-300-9502/learning/advanced-antivirus-evasion-14696/advanced-antivirus-evasion-15017#fnref-local_id_325-13)
